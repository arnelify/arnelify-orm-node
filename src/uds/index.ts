...
class ArnelifyUDS {
      ...
      socket.on('data', async (block: Buffer<ArrayBuffer>): Promise<void> => {
        ...
        while (buffer.length > 0) {
          ...
          if (buffer.length >= size) {
            ...
            if (!uuid) {
              this.#logger("The 'uuid' is missing in the message.", true);
              process.exit(1);
            }

            if (!content) {
              this.#logger("The 'content' is missing in the message.", true);
              process.exit(1);
            }
   ...
   */
  #logger: (message: string, isError: boolean) => void = (message: string, isError: boolean): void => {
